...
-- 8) Owner XOR rules via triggers (phpMyAdmin/MySQL-safe alternative to CHECK + FK).
DROP TRIGGER IF EXISTS trg_location_profiles_owner_bi;
DROP TRIGGER IF EXISTS trg_location_profiles_owner_bu;
DROP TRIGGER IF EXISTS trg_carts_owner_bi;
DROP TRIGGER IF EXISTS trg_carts_owner_bu;
DROP TRIGGER IF EXISTS trg_customer_orders_owner_bi;
DROP TRIGGER IF EXISTS trg_customer_orders_owner_bu;

DELIMITER $$

CREATE TRIGGER trg_location_profiles_owner_bi
BEFORE INSERT ON location_profiles
FOR EACH ROW
BEGIN
  IF (NEW.user_id IS NULL AND NEW.guest_id IS NULL)
     OR (NEW.user_id IS NOT NULL AND NEW.guest_id IS NOT NULL) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'location_profiles: set exactly one of user_id or guest_id';
  END IF;
END$$

CREATE TRIGGER trg_location_profiles_owner_bu
...
CREATE TRIGGER trg_carts_owner_bi
...
BEGIN
  IF (NEW.user_id IS NULL AND NEW.guest_id IS NULL)
     OR (NEW.user_id IS NOT NULL AND NEW.guest_id IS NOT NULL) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'carts: set exactly one of user_id or guest_id';
  END IF;
END$$

CREATE TRIGGER trg_carts_owner_bu
...
CREATE TRIGGER trg_customer_orders_owner_bi
...
BEGIN
  IF (NEW.user_id IS NULL AND NEW.guest_id IS NULL)
     OR (NEW.user_id IS NOT NULL AND NEW.guest_id IS NOT NULL) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'customer_orders: set exactly one of user_id or guest_id';
  END IF;
END$$

CREATE TRIGGER trg_customer_orders_owner_bu
